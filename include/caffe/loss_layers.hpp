...
class SemiRankLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  Blob<Dtype> fea_diff_;
  Blob<Dtype> data_diff_;
  Blob<Dtype> simi_rate;
  Blob<Dtype> label_cnt_;
  Blob<Dtype> pseudo_label_;
  Blob<Dtype> pseudo_label_false_;
  ...
  vector<int> vec_anchor_pl;
  vector<int> vec_pos_pl;
  vector<int> vec_neg_pl;
  int iter_count;
};
...
/**
 * @brief Semi-Multi-label Softmax Layer With Loss
 */

template <typename Dtype>
class SemiMLabelSoftmaxLossLayer : public LossLayer<Dtype> {
 public:
  explicit SemiMLabelSoftmaxLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const {return "SemiMLabelSoftmaxLoss";}
  virtual inline int ExactNumBottomBlobs() const { return -1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline int MaxTopBlobs() const { return 2; }

 protected:
  /// @copydoc SemiMLabelSoftmaxLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
